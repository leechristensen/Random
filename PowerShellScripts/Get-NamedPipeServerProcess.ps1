...
$FunctionDefinitions = @(
    #(func kernel32 CreateFileW ([IntPtr]) @([String],[System.IO.FileAccess],[System.IO.FileShare],[IntPtr],[System.IO.FileMode],[System.IO.FileAttributes],[IntPtr])),
    (func kernel32 CreateFileW ([IntPtr]) @([String],[Int],[Int],[IntPtr],[Int],[Int],[IntPtr])),
    (func kernel32 CloseHandle ([Boolean]) @([IntPtr])),
    (func kernel32 GetNamedPipeServerProcessId ([Int]) @([IntPtr], [UInt32].MakeByRefType())),
    (func kernel32 GetNamedPipeServerSessionId ([Int]) @([IntPtr], [UInt32].MakeByRefType())),
...
#>
    ...
    { 
        ...
        $Output.PSObject.TypeNames.Insert(0, 'CimSweep.NamedPipe')
	    Write-Output $Output
...
#>
function Get-NamedPipeServerProcess
{
    $Pipes = Get-PipeList
    ...
    foreach($Pipe in $Pipes)
    {
        ...
        {
            $Handle = $Kernel32::CreateFileW("\\.\pipe\$($Pipe.Name)", 0x80000000, 1, 0, 3, 0x02000000,0)
            

            if($Handle -eq -1)
            {
                #Write-Warning "Could not open $($Pipe.Name)"
                New-Object PSObject -Property @{
                    Name = $Pipe.Name
                    Instance = $Pipe.Instances
                    ProcessId = $ProcessId
                    Process = $PipeProcess.Path
                }
                continue    
            }

            $ProcessId = $null
            $ret = $Kernel32::GetNamedPipeServerProcessId($Handle, [ref]$ProcessId)

            if($ret -ne 0 -and $ProcessId -ne $null)
            {
                $PipeProcess = Get-Process -Id $ProcessId -ErrorAction SilentlyContinue
            }
            else
            {
                $Error = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
                $Error = $kernel32::GetLastError()
                $PipeProcess = "Error: $Error"
            }

            $Sessionid = $null
            $Ret = $Kernel32::GetNamedPipeServerSessionId($Handle, [ref]$Sessionid)

            $ret = $Kernel32::CloseHandle($Handle)
            if($ret -eq $false)
            {
                Write-Warning "Could not close handle to $($Pipe.Name)"
            }
        }
        else
        {
            $PipeProcess = "GetNamedPipeServerProcessId API unavailable"
        }
        ...
        New-Object PSObject -Property @{
            ...
            Instance = $Pipe.Instances
            ProcessId = $ProcessId
			      ProcessName = $PipeProcess.Name
            Process = $PipeProcess.Path
            SessionId = $Sessionidg
        }
    }
}
